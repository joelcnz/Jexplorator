...
public:
	@property {
		...
		Vec plopPoint() { return _plopPoint; }
		...
		Vec resetPos() { return _resetPos; }
		void resetPos(Vec p) { _resetPos = p; }
		...
		version(none) {
			...
			override void scrn(Vector!int scrn0) { _portal._scrn = scrn0; };
		}
	...
	this(int id, Portal portal, Uint8[] keys) { // if (g_keys[ _keys[Key.shoot] g_keys[keys[Key.shoot]].keyTrigger].keyTrigger)
		...
		_portal = portal;
		_portal.scrn = Vector!int(0,0);
		_keys = keys;
		bullitProof = true;
		_resetPos = Vec(0,0);
		portal.resetPosScrn = Vector!int(0,0);
		doGrace;

		// guyRight1, guyRight2
		foreach(i; [TileName.guyWalkRight1, TileName.guyWalkRight2]) {
			_framesForward ~= new Sprite();
			_framesForward[$-1] = inf[cast(TileName)i];
			/+
			_framesForward ~= new Sprite;
		...
		foreach(i; [TileName.guyWalkLeft1, TileName.guyWalkLeft2]) {
			...
			_framesBackward[$-1] = inf[cast(TileName)i];
			/+
		...
		foreach(identify; [TileName.plopUp, TileName.plopFall]) {
			...
			_plopping[$-1] = inf[cast(TileName)identify];
			/+
			with(_plopping[$-1]) {
				setTexture = g_texture;
		...
		_gunAimLeft = new Sprite();
		_gunAimRight = new Sprite();
		_gunAimLeft = inf[TileName.guyAimLeft];
		_gunAimRight = inf[TileName.guyAimRight];
		...
		_gunTriggerLeft = new Sprite();
		_gunTriggerRight = new Sprite();
		_gunTriggerLeft = inf[TileName.guyTiggerPulledLeft];
		_gunTriggerRight = inf[TileName.guyTiggerPulledRight];
		...
		_squatTriggerLeft = new Sprite();
		_squatTriggerRight = new Sprite();
		_squatTriggerLeft = inf[TileName.duckLeft1];
		_squatTriggerRight = inf[TileName.duckRight1];
		...
		_squatAimLeft = new Sprite();
		_squatAimRight = new Sprite();
		_squatAimLeft = inf[TileName.duckLeft2];
		_squatAimRight = inf[TileName.duckRight2];
		...
		banner.setup([""], Vec(320 * id, 160), Vec(320,20));
	}

	void doResetPos(Vec resetPos) {
		_resetPos = resetPos;
	}
	...
	bool placeGuy(Vector!int scrn, Vec pos) {
	...
	bool isLadder() { //#if bottomLeft or bottomRight in on a ladder and not left in bottom, and not right bottom
		with(TileName)
			...
			return (hits(bottomLeft, [ladder]) || hits(bottomRight, [ladder])) &&
				! hits(Vec(_pos.x, _pos.y + g_spriteSize - 1), [ladder]) &&
				! hits(Vec(_pos.x + g_spriteSize - 1, _pos.y + g_spriteSize - 1), [ladder]);
	}

	//#this is stink, this can't go up in 'mover'
	override TileName getPos(in Vec v, Layer layer = Layer.normal) {
	...
	void process() {
		...
		if (_rocketState == Rocket.goingUp) {
			...
			if (_pos.y < -g_spriteSize) {
				...
				g_inputJex.addToHistory(g_score.winner);
				_escapeStatus = GuyEscapeStatus.escaped;
				//_escaped.status = EscapeStatus.escaped;
				_dashBoard.banner = "Gotten away in rocket!";
		...
		if (getPos(_pos, Layer.front) == TileName.rocket &&
			...
			_dashBoard.banner = "Getting away in rocket!";
			g_jsounds[Snd.rocket].play(false);
		...
		if (_climbing == Climbing.no &&
			...
			p1n2GunTrigger) {
				if (_gun == Gun.aiming) {
					...
					g_jsounds[Snd.shoot].play(false);
					...
					if (_facing == Facing.right) {
						g_bullits ~= new Bullit(this, _portal.scrn, _pos + Vec(g_spriteSize - 2, height),
							Vec(g_pixelsx * 2, 0));
					}
					if (_facing == Facing.left) {
						g_bullits ~= new Bullit(this, _portal.scrn, _pos + Vec(-2, height), Vec(-g_pixelsx * 2, 0));
					}
				} else
					_gun = Gun.aiming;
		}
		...
		if (hits(mid, [TileName.diamond])) {
			...
			_dashBoard.banner = "Diamond Collected";
			if (g_score.targetDiamondsQ && g_gotTargetDiamonds == false) {
				_dashBoard.banner = "Target diamonds collected!";
				...
				_dashBoard.banner = "All diamonds collected!";
		...
		if (_dying == Dying.alive && (hits(Vec(_pos.x + 7, _pos.y + 7), [TileName.spikes]) ||
			hits(Vec(_pos.x + g_spriteSize - 7, _pos.y + 7), [TileName.spikes]))) {
			die;
			_plopPoint = pos;
			if (hits(Vec(_pos.x + 7, _pos.y + 7), [TileName.spikes]))
				g_mouse.setTile(_portal, Vec(_pos.x + 7, _pos.y + 7), TileName.gap, Layer.normal);
			else
				g_mouse.setTile(_portal, Vec(_pos.x + g_spriteSize - 7, _pos.y + 7), TileName.gap, Layer.normal);
		}
		...
		void moveLeft() {
			...
			with(TileName)
				if (! hits(leftTop, _blocks) &&
					...
					_pos = Vec(_pos.x - g_pixelsx, _pos.y);
					if (_pos.x < 0) {
						if (_portal.scrn.x - 1 >= 0) {
							_pos.x = 10 * g_spriteSize - g_spriteSize;
							with(_portal) {
								scrn = Vector!int(scrn.x - 1, scrn.y);
							} // with
						} else {
							_pos = Vec(_pos.x + g_pixelsx, _pos.y);
						}
		...
		void moveRight() {
			...
			with(TileName)
				if (! hits(rightTop, _blocks) &&
					...
					//if (hitOther)
					//	onTop = true;
					_pos = Vec(_pos.x + 2, _pos.y);
					//if (onTop == false && hitOther)
					//	_pos = Vec(_pos.x - 1, _pos.y);
		...
		if (g_keys[ _keys[Key.up] ].keyPressed) {
			if (hits(Vec(_pos.x + g_spriteSize / 2, _pos.y + g_spriteSize - 1), [TileName.ladder]) ||
				hits(Vec(_pos.x + g_spriteSize / 2, _pos.y), [TileName.ladder]))
		...
		if (_climbing == Climbing.up) {
			...
			if (hits(topLeft, _blocks) || hits(topRight, _blocks))
				_pos = Vec(_pos.x, _pos.y + g_pixelsy); // back track
			else {
				_climbingPreFrame++;
			...
			if (! hits(Vec(_pos.x + 16, _pos.y + g_spriteSize - 1), [TileName.ladder]) &&
				! hits(Vec(_pos.x + 16, _pos.y), [TileName.ladder]) ) {
		...
		if ((_climbing == Climbing.up || _climbing == Climbing.down) &&
			! g_keys[ _keys[Key.left] ].keyPressed &&
			! g_keys[ _keys[Key.right] ].keyPressed) {
			if (! hits(leftTop + Vec(1,0), [TileName.ladder]) &&
				! hits(leftBottom + Vec(1,0), [TileName.ladder]))
					_pos = Vec(_pos.x + 2, _pos.y);

			if (! hits(rightTop + Vec(-1, 0), [TileName.ladder]) &&
				! hits(rightBottom + Vec(-1, 0), [TileName.ladder]))
				_pos = Vec(_pos.x - g_pixelsx, _pos.y);
		}
		
		// move up if block on feet
		if ((hits(_pos + Vec(0, g_spriteSize - 1), _blocks) ||
			hits(_pos + Vec(g_spriteSize - 1, g_spriteSize - 1), _blocks)) &&
			! hits(_pos + Vec(0, g_spriteSize - 2), _blocks) &&
				! hits(_pos + Vec(g_spriteSize - 1, g_spriteSize - 2), _blocks)) {
		...
		if (_climbing == Climbing.down) {
			...
			if (hits(bottomLeft, _blocks) || hits(bottomRight, _blocks)) {
				_pos += Vec(0, -g_pixelsy); // back track
				//_climbing = Climbing.no;
			}
			else {
				_climbingPreFrame++;
			...
			if (! hits(Vec(_pos.x + g_spriteSize / 2, _pos.y + g_spriteSize), [TileName.ladder]) &&
				! hits(Vec(_pos.x + g_spriteSize / 2, _pos.y), [TileName.ladder])) {
				_climbing = Climbing.no;
			}
		...
		if (g_keys[ _keys[Key.down] ].keyPressed) {
			if ((hits(Vec(_pos.x + g_spriteSize / 2, _pos.y + g_spriteSize), [TileName.ladder]) ||
				hits(Vec(_pos.x + g_spriteSize / 2, _pos.y - 1), [TileName.ladder])) &&
				! hits(Vec(_pos.x + g_spriteSize / 2, _pos.y + 2), _blocks))
				_climbing = Climbing.down,
				_jumping = Jumping.no;
			else {
				_stateUpDown = StateUpDown.falling;
			...
			if (! g_keys[ _keys[Key.left] ].keyPressed &&
				...
				if (hits(bottomLeft, _blocks) ||
					...
					if (! isLadder)
						_climbing = Climbing.no;
					_pos = Vec(_pos.x, cast(int)_pos.y / cast(int)g_spriteSize * cast(int)g_spriteSize);
				} else {
					_gunDucked = GunDucked.notDucked;
				}
			}
		...
		if (_stateUpDown != StateUpDown.rising && _stateUpDown != StateUpDown.gliding && _climbing == Climbing.no) {
			...
			with(TileName) {
				if (! hits(bottomLeft, _blocks) &&
					...
					if (! isLadder &&
						! (hits(Vec(_pos.x + g_spriteSize / 2, _pos.y + g_spriteSize), [ladder]) &&
							hits(Vec(_pos.x - 1, _pos.y + g_spriteSize), _blocks) &&
							hits(Vec(_pos.x + g_spriteSize, _pos.y + g_spriteSize), _blocks))) {
						_stateUpDown = StateUpDown.falling;
						_pos += Vec(0, g_pixelsy);
						if (_pos.y + g_spriteSize >= 10 * g_spriteSize) { // if edge of screen
							if (_portal.scrn.y + 1 < g_scrnDim.y) {
								...
								_pos = Vec(_pos.x, _pos.y - g_spriteSize);
							}
						}
					} else
						_stateUpDown = StateUpDown.nothing;
		...
		if (_stateUpDown == StateUpDown.rising) {
				...
				if (! hits(topLeft, _blocks) && 
					...
					if (_pos.y < 0) {
						if (_portal.scrn.y - 1 < 0) {
							...
							_risingCount += cast(int)(g_spriteSize - g_pixelsy * 2);
							doGrace;
						}
					} else {
							_pos += Vec(0, -g_pixelsy);
			...
			//#stop on a block in a jump up a screen
			if ((hits(topLeft + Vec(0, 1), _blocks) ||
				 hits(topRight + Vec(0, 1), _blocks)) &&
				! hits(topLeft + Vec(0, 2), _blocks) &&
				! hits(topRight + Vec(0, 2), _blocks))
				_pos += Vec(0, g_pixelsy);
		} // rising
...
//			}
		...
		auto posWas = _pos;
		_pos = _id == player1 ? Vec(_pos.x, _pos.y) : Vec(_pos.x + _portal.pos.x, _pos.y);
		...
		if (g_guys[other]._portal.scrn == _portal.scrn) {
			if (other == 1) {
				with(g_guys[other]) {
					auto gposWas = _pos;
					_pos = Vec(_pos.x - _portal.pos.x, _pos.y);
			...
			if (other == 0 && skip == false) {
				...
				with(g_guys[other]) {
					_pos = Vec(320 + _pos.x, _pos.y); //#magic number
					draw(false, false);
					_pos = gposWas;
				}
			}
		...
		final switch(_rocketState) {
			...
			break;
			case Rocket.goingUp:
				gGraph.draw(_rocket.image, _pos);
				//g_window.draw(_rocket);
				//SDL_RenderCopy(gRenderer, spr, null, &mPos);
			break;
		}
					...
					case StateWalking.walking:
						final switch(_facing) {
							case Facing.right:
								...
								gGraph.draw(_framesForward[_currentFrame].image, _pos);
							break;
							case Facing.left:
								//g_window.draw(_framesBackward[_currentFrame]);
								gGraph.draw(_framesBackward[_currentFrame].image,_pos);
							break;
						}
						break;
					case StateWalking.standingStill:
						final switch(_facing) {
							case Facing.right:
								...
								gGraph.draw(_framesForward[0].image,_pos);
							break;
							case Facing.left:
								//g_window.draw(_framesBackward[0]);
								gGraph.draw(_framesBackward[0].image,_pos);
							break;
						}
					break;
				}
				...
				case Gun.aiming:
					final switch(_gunDucked) {
						case GunDucked.notDucked:
							...
							(_facing == Facing.right ? gGraph.draw(_gunAimRight.image,_pos) : gGraph.draw(_gunAimLeft.image,_pos));
						break;
						case GunDucked.ducked:
							//(_facing == Facing.right ? g_window.draw(_squatAimRight) : g_window.draw(_squatAimLeft));
							(_facing == Facing.right ? gGraph.draw(_squatAimRight.image,_pos) : gGraph.draw(_squatAimLeft.image,_pos));
						break;
					}
				break;
				case Gun.trigger:
					final switch(_gunDucked) {
						case GunDucked.notDucked:
							...
							(_facing == Facing.right ? gGraph.draw(_gunTriggerRight.image,_pos) : gGraph.draw(_gunTriggerLeft.image,_pos));
						break;
						case GunDucked.ducked:
							//(_facing == Facing.right ? g_window.draw(_squatTriggerRight) : g_window.draw(_squatTriggerLeft));
							(_facing == Facing.right ? gGraph.draw(_squatTriggerRight.image,_pos) : gGraph.draw(_squatTriggerLeft.image,_pos));
						break;
					}
				break;
			}
			...
			if (_gun == Gun.normal)
				final switch(_stateUpDown) {
					...
					case StateUpDown.gliding:
						final switch(_facing) {
							case Facing.right:
								gGraph.draw(_framesForward[1].image,_pos);
							break;
							case Facing.left:
								gGraph.draw(_framesBackward[1].image,_pos);
							break;
						}
					break;
		...
		if (_dying == Dying.alive)
			final switch(_climbing) {
				...
				case Climbing.up:
				case Climbing.down:
					gGraph.draw(_climb[_climbingFrame].image,_pos);
				break;
			}
		...
		final switch(_dying) {
			...
			case Dying.dyingUp: gGraph.draw(_plopping[0].image,_pos); _dashBoard.banner = "Plop up"; break;
			case Dying.dyingDown: gGraph.draw(_plopping[1].image,_pos); _dashBoard.banner = "Plop down"; break;
		}
	...
	void updateScreenVerseRef() {
		...
		if (_portal.scrn.x >= 0 && _portal.scrn.x < g_scrnDim.x &&
			...
			_dashBoard.banner = g_screens[_portal.scrn.y][_portal.scrn.x].verseRef;
